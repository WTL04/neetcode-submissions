"""
# Definition for a Node.
class Node:
    def __init__(self, val: Optional[int] = None, children: Optional[List['Node']] = None):
        self.val = val
        self.children = children
"""

class Solution:
    def postorder(self, root: 'Node') -> List[int]:
        """
        understanding:
            post order = left -> right -> parent
                in other words, search through all children before adding the parents

            each node has n amount of children, there are no left or rights

            the children are stored in as a 'list of nodes', while the function only takes a single node as a parameter

            return an array of the search results in postorder
        
        match: 
            we can traverse post order using recursive DFS


        plan:

            base case: if empty node, return empty array

            keep an result array to track searched nodes
            iterate through all children in parent
                extend result with newly searched child nodes

            return results + parent value


        """

        if not root:
            return []

        result = []

        for child in root.children:
            result.extend(self.postorder(child))

        return result + [root.val]

        """
        eval:
         time: O(n), where n is the total number of nodes, because we are searching through all nodes
         space: O(n), where n is the total number of nodes, because we are storing all searched nodes
        """