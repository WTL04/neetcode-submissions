class Solution:
    def minSubArrayLen(self, target: int, nums: List[int]) -> int:
        ...
        expand window until end of array
            if subarray is greater than or equal to target
                record current window length 
                compare to minimum_length
                shrink window

        if minimum_length hasn't changed:
            return 0, there is no subarray
        ...
        """
        evaluate:
            time: O(n), where n is the lenght of nums, sliding window means 1 pass
            space: O(1), nothing stored in memory
        """
