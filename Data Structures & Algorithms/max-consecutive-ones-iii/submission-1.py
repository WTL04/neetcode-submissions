class Solution:
    def longestOnes(self, nums: List[int], k: int) -> int:
        ...
        """
        - evaluate:
            runtime: O(n) where n is the length of nums, using the sliding window, only one pass is nessesary through the array
            space: O(1), since there is no data structured being used to store anything
        """